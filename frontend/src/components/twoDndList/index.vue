<template>
  <div class="twoDndList">
    <div class="twoDndList-list" :style="{width:width1}">
      <h3>{{list1Title}}</h3>
      <draggable :list="list1" class="dragArea" :options="{group:'questions'}">
        <div class="list-complete-item" v-for="element in list1" :key='element.id'>
          <div class="list-complete-item-handle">{{element.description}}</div>
    ...
    <div class="twoDndList-list" :style="{width:width2}">
      <h3>{{list2Title}}</h3>
      <draggable :list="filterList2" class="dragArea" :options="{group:'questions'}">
        <div class="list-complete-item" v-for="element in filterList2" :key='element.id'>
          <div class='list-complete-item-handle2' @click="pushEle(element)"> {{element.description}}</div>
        </div>
      </draggable>
    </div>
  </div>
